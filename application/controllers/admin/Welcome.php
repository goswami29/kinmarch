...
class Welcome extends CI_Controller {
    ...
    function index() {
        ...
        $data['list'] = $this->db->select('*')
                    ->get('welcome')->result_array();
        $this->load->view('admin/welcomelist', $data);
    }
    ...
    function addwelcome() {  
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($this->form_validation->run() == FALSE) {
                $this->load->view('admin/welcomeform', $data);
            } 
            else
            {  
                ...
                if (isset($_POST['id']) && !empty($_POST['id'])) {
                    $this->Auth_model->update_data($data1, array('id' => $_POST['id']), 'welcome');
    ...
    function edit($id) {
        ...
        $data['active'] = 1;
        $where = array('id' => $id);
        $data['edit'] = $this->db->where($where)->get('welcome')->row_array();
        $this->load->view('admin/welcomeform', $data);
    }
